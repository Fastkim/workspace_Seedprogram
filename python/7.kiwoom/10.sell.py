...
class kiwoom(QAxWidget):
    ...
    def onResponse(self, screenno, rqname, trcode, rname, next):
        ...
        elif rqname == '잔고':  # rqname : request name
            box = []
            for i in range(self.GetRepeatCnt(trcode, rqname)):
    ...
    def order(self, rqname, orderType, stockCode, q, p, gubun, orderNo=''): # 가격 : p , 0이면 시장가로 해라. , q : 주문수량
    ...
    def buy(self, stockCode):
        ...
        if not candles.empty:
            ...
            for i in range(len(candles) - 1): # 마지막 index
            ...
            rf.fit(data, target) # 훈련시키기, 입력값주고 정답주고
