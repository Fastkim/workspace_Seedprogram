...
public class MyList {
	public static void main(String[] args) { // List interface는 자식이 여러개있지만 그중 하나가 ArrayList
		List<Integer> list = new ArrayList<>(); // diamond 타입, ArrayList<Integer(생략)> : 코드를 간결하게 하기위해 생략되었다.
		list.add(1); // add라는 method는 List라는 interface에 저장되어있지만, 오버라이드 규칙에 따라 ArrayList에서 실행.
		list.add(2);
		list.add(3); // layberer (노동자) 타입?
		list.add(1); // list에서 데이터를 write할때는 add(insert)메서드 호출
		
		System.out.println(list);
		// [1, 2, 3, 1] key,value 로 이루어졌다. key = index, key는 자동생성
		...
		System.out.println(list.get(0)); // list에서 데이터를 read할때 get()
