...
user.height =  200 //property를 add하는 과정.
...
console.log(Object.values(user)) // values : String or Number를 array타입으로 리턴한다. value : 객체가 담고있는 property value을 array로 리턴해준다. 
...
let street
street = user?.city?.street // ?.(optional chaining) : 앞의 주어(variable)가 null || undefined 이면 undefined로 출력. 앱을 죽지않게 하는게 목적.
console.log(street)

street = user?.city?.street ?? 'where' // ??(nullish coalescing operator) 'default' : undefined 대신 출력할 기본값
console.log(street)
