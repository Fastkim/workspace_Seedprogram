...
//
let a = [1, 2] // 생성자 parameter로 넣은 것이다. let a = new Array(1, 2)
let b = [2, 3]
let c = a.concat(b) // 합집합 sql의 union all 같네. 중복제거하지 않음.
console.log(c)
...
console.log(arr)
console.log(arr.pop()) // arr 원소 추출, 가장 마지막에 들어간 push(3)가 추출되었다.
...
//
arr = [4, 3, 1, 2]
let s = arr.sort() // sort()는 arr을 write 하고있다. update
...
const elements = items.map((item) => // map: 배열의 모든 원소들을 원하는형식으로 불러온다.
    `<div>${item.itemName}: ${item.price}</div>`)
console.log(elements)

const totalPrice = items.reduce((total, item) => { // array 원소를 하나씩 꺼내서 함수에 투입 리턴
            total += item.price
            return total
