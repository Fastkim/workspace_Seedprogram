...
employee_id number(3) primary key, -- primary key : not null + unique
...
insert into emps values(501, null, 'good@gamil.com', 6000, 100); -- 2번째 컬럼의 not null 제약조건 위배
...
drop table gu cascade constraints; -- 복습을 위한 코드
drop table dong cascade constraints; -- 복습을 위한 코드
drop table dong2 cascade constraints; -- 복습을 위한 코드
...
gu_id number(3) references gu(gu_id) on delete cascade); -- gu(gu_id) 부모테이블에 delete 이벤트가 발생하면 자식값도 삭제할거야

create table dong2(
dong_id number(4) primary key,
dong_name varchar2(12) not null,
gu_id number(3) references gu(gu_id) on delete set null); -- gu(gu_id) 부모테이블에 delete 이벤트가 발생하면 자식필드 null값으로 셋팅할거야
...
drop table a cascade constraints; -- 복습을 위한 코드
drop table b cascade constraints; -- 복습을 위한 코드
...
insert into b values(32, 9); -- parent table에 9라는 데이터가 없다. 에러

alter table b disable constraint b_aid_fk; -- foreign key 제약조건을 잠시 잠재운다.
insert into b values(32, 9);

alter table b enable constraint b_aid_fk; -- 에러, 이미 부모테이블에 없는 aid : 9 값을 가지고있으므로 무결성이 깨졌다.
alter table b enable novalidate constraint b_aid_fk;

insert into b values(33, 8);

drop table sub_departments; -- 복습을 위한 코드

create table sub_departments as -- table 복사해오기, 원본은 내비두고 테스트를 위해 복제테이블 생성할때 유용
...
drop table users cascade constraints; -- 복습을 위한 코드
...
alter table users add(user_name varchar2(10)); -- 칼럼추가
desc users

alter table users modify(user_name number(7)); -- 데이터타입 변경
desc users

alter table users drop column user_name; -- 컬럼삭제
desc users
---

insert into users values(1);

alter table users read only; -- 수정이 불가능하게 lock하는 명령문, 읽기만 가능하게
insert into users values(2);

alter table users read write; -- 읽고,쓰기가 가능하게 바꾸는 명령문
insert into users values(2);
