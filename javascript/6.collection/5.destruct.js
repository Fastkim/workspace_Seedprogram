...
let [one, two] = [1, 2, 3] // 할당하는쪽에서 overflow할경우 무시, queue 알고리즘 작동
console.log(one, two)
...
let {title, height, width} = options // 객체의 value return
console.log(title, height, width);

({title, width, height} = options) // 순서에 맞추지않고 destructing 하는법, 단 key와 variable name을 일치
console.log(title, height, width)
